...
class DemosTableViewController: UITableViewController {
    ...
    let tableRows = ["About", "Read/Write Files", "Read Web Page", "RESTful GET"]
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == 1 {
            let vc = storyboard?.instantiateViewController(withIdentifier: "TextVC")
            navigationController?.pushViewController(vc!, animated: true)
            return
        }
        
        if indexPath.row == 2 {
            let vc = storyboard?.instantiateViewController(withIdentifier: "TextVC") as! TextViewController
            vc.textViewMode = .Web
            navigationController?.pushViewController(vc, animated: true)
        }
    }
